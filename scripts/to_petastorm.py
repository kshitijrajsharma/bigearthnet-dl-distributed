...
BIGEARTH_IDS = [111, 112, 121, 122, 123, 124, 131, 132, 133, 141, 142, 211, 212, 213, 221, 222, 223, 231, 241, 242, 243, 244, 311, 312, 313, 321, 322, 323, 324, 331, 332, 333, 334, 335, 411, 412, 421, 422, 423, 511, 512, 521, 522, 523, 999]
...
def process_partition(rows, schema):
    ...
    for row in rows:
        try:
            def clean(p):
                return p.replace("s3a://", "").replace("s3://", "") # s3fs doesn't support s3a
            ...
            raw_files = fs.cat(list(path_map.values())) # fs has this cool api that can fetch multiple files in parallel https://s3fs.readthedocs.io/en/latest/api.html#s3fs.core.S3FileSystem.cat
...
def convert_to_petastorm(metadata_path, output_dir, fraction=1.0, args=None):
    ...
    try:
        for name, split_df in splits.items():
            ...
            bytes_per_row = (120*120*6*4) + (120*120)  # it is coming from above schema , 4 image bands with 120/120 image size and labels is basically 120/120
            rows_per_file = int((args.target_file_mb * 1024**2) / (bytes_per_row * 0.4)) # 0.4 compression factor, i am putting this as approximate , idk honestly how much is the compression factor as it depends upon the type of the data being compressed 
            partitions = max(1, len(split_df) // rows_per_file)
            
            profiler.log(f"Processing {name}: {len(split_df)} rows, {partitions} partitions")
...
def main():
    ...
    parser.add_argument("--meta", default="s3a://ubs-homes/erasmus/raj/dlproject/metadata_with_paths.parquet")
    parser.add_argument("--out", default="s3a://ubs-homes/erasmus/raj/dlproject/testpercent/petastorm")
