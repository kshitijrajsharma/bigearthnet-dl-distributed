...
def convert_to_petastorm(
    ...
    InputSchema = Unischema(
        ...
        [
            UnischemaField(
                "patch_id_int", np.int32, (), ScalarCodec(IntegerType()), False
            ),
            UnischemaField(
                "input_data", np.float32, input_shape, NdarrayCodec(), False
            ),
...
def main():
    ...
    parser.add_argument(
        ...
        nargs=2,
        default=[120, 120],
        help="Target patch size H W",
    )
    parser.add_argument(
        "--executor-mem", required=False, help="executor memory", default="8g"
    )
    parser.add_argument(
